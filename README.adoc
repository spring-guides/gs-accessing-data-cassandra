...
. Click *Dependencies* and select *Spring Data for Apache Cassandra*, *Docker Compose*, and *Testcontainers*.
. Click *Generate*.
...
The `spring.cassandra.schema-action` property defines the schema action to take at startup and can be `none`, `create`, `create-if-not-exists`, `recreate` or `recreate-drop-unused`. We use `create-if-not-exists` to create the required schema. See the
...
[source,xml]
----
include::complete/pom.xml[tag=astra]
----
====

NOTE: For Gradle, add the following to your `build.gradle` file:

[source,gradle]
----
include::complete/build.gradle[tag=astra]
----
...
If you prefer to run Cassandra locally in a containerized environment add the following `compose.yaml` at the root of the project:
...
Notice the `accessing_data_cassandra` container name.

You can use the regular Docker compose command, typically:
...
After the container has started, you need to create a Keyspace.
Access the Cassandra query language shell as follows

[source,bash]
----
docker exec -it accessing_data_cassandra bash -c "cqlsh -u cassandra -p cassandra"
----
...
Alternatively, for a convenient bundle of Cassandra and related Kubernetes ecosystem projects, you can spin up a single node Cassandra cluster on https://docs.k8ssandra.io[K8ssandra] in about 10 minutes.
...
To see how this works, create a repository interface that queries `Vet` entities, as the following listing (in `src/main/java/com/example/accessingdatacassandra/VetRepository.java`) shows:
...
`VetRepository` extends the `CrudRepository` interface and specifies types for the generic type parameters for both the value and the key that the repository works with -- `Vet` and `UUID`, respectively. This interface comes with many operations, including basic CRUD (Create, Read, Update, Delete) and simple query (such as `findById(..)`) data access operations.

[NOTE]
====
Spring Data Cassandra also ships with https://docs.spring.io/spring-data/cassandra/docs/current/api/org/springframework/data/cassandra/repository/CassandraRepository.html[`CassandraRepository`], an interface that exposes Cassandra-specific support for partitioning and compound keys.
====
