...
Follow the instructions in the following link to create a database and keystore named `spring-cassandra`.
...
Cassandra denormalizes data and does not need table joins like SQL does, which allows you to retrieve data much faster. For that reason, we have modelled our `specialties` as a `Set<String>`.
...
Congratulations! You've just developed a Spring application that use Spring Data Cassandra to access distributed data.
